...
def parse_args():
    ...
    parser.add_argument("--exp_dir", type=str, default="F:/program_output/SCNN/exp0_scnn_no_left")  #./experiments/exp0
...
tensorboard = SummaryWriter(exp_dir)
...
train_loader = DataLoader(train_dataset, batch_size=exp_cfg['dataset']['batch_size'], shuffle=True, collate_fn=train_dataset.collate, num_workers=4)
...
def train(epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        tensorboard.add_scalar(exp_name + "/train_loss", train_loss, iter_idx)
        tensorboard.add_scalar(exp_name + "/train_loss_seg", train_loss_seg, iter_idx)
        tensorboard.add_scalar(exp_name + "/train_loss_exist", train_loss_exist, iter_idx)
        tensorboard.add_scalar(exp_name + "/learning_rate", lr, iter_idx)
...
def main():
    ...
    for epoch in range(start_epoch, 8):   # exp_cfg['MAX_EPOCHES']
        train(epoch)
        if epoch % 1 == 0:
